...
const EventDetails = () => {
  ...
  const handleCreateSchedule = async () => {
    ...
    const scheduleFunctions = newFunctions.filter(
      (func) => func.funcao.trim() !== "" || func.nome.trim() !== ""
    );
  ...
  const handleDeleteSchedule = async () => {
    if (
      window.confirm(
        "Tem certeza que deseja excluir esta escala? Esta ação é irreversível."
      )
    ) {
      try {
        ...
        {[
          {
            ...
            value: eventData.data
              ? eventData.data.split("-").reverse().join("/")
              : eventData.data,
          },
          ...
          <div key={label}>
            <h2 className="text-left text-lg font-bold text-[#383838]">
              {label}:
            </h2>
    ...
    <MobileLayout>
      ...
      <section className="px-4 mt-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-left text-2xl font-bold text-[#383838]">
            Escala do evento
          </h2>
          {isAdmin && (
            <button
          ...
          <div className="space-y-4">
            {scheduleData.funcoes.map((item, index) => (
              <div key={index} className="mb-4">
                ...
                <p className="text-[#706382]">{item.nome}</p>
              </div>
            ))}
          </div>
        ) : (
